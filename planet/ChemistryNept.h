 ...
 * Standalone chemistry class for the Neptune model.
...
class ChemistryNept {
    friend class cNeptuneModel;

public:
    explicit ChemistryNept(cNeptuneModel& model) : m(model) {}
    ~ChemistryNept() = default;
    ChemistryNept(const ChemistryNept&) = delete;
    ...
    {
        using namespace std;
        cout << endl << "      ATNEPT: ChemMassRateNept" << endl;
        ...
        for(int k = 1; k < km-1; k++){
            for(int j = 1; j < jm-1; j++){
                for(int i = 1; i < im-1; i++){
                    double t_u = m.t.x[i][j][k] * m.t_ref;              // K
                    double kf  = react_rate_const(t_u, T_d, A, B);      // forward reaction rate
                    double kb  = kf / keq;                              // backward reaction rate
                    if((t_u >= m.t_00_nh4sh) && (t_u <= m.t_0_nh4sh)){
                        ...
                        const double R_diff  = kf * c_nh3 * c_h2s - kb * c_nh4sh;  // net reaction rate

                        m.w_h2s.x[i][j][k]   = -m.m_h2s   * R_diff;
                        m.w_nh3.x[i][j][k]   = -m.m_nh3   * R_diff;     // mass source term
                        m.w_nh4sh.x[i][j][k] =  m.m_nh4sh * R_diff;
                    }

                    m.massflux_h2s.x[i][j][k]   = m.w_h2s.x[i][j][k]   - m.difflux_h2s.x[i][j][k];  // net mass flux
        ...
        printf(" time measured: %.3f seconds for ChemMassRateNept\n", elapsed.count() * 1e-9);

        cout << "      ATNEPT: ChemMassRateNept ended" << endl;
        return;
    }

    // -----------------------------------------------------------------------
    // The TVD limiter is the SHARED FluxLimiter<Planet> template — ATNEPT's hand-written copy
    // was the same code as ATSAT's and ATJUP's, differing only where each model answers two
    // questions the shared version now asks through hooks: rm = m.rad.z[i] against
    // m.metricRadius(m.rad.z[i]), and a local `constexpr sinthe_min = 0.4` against
    // ATPhys::polar_divisor_floor<Planet>(). Both reproduce ATNEPT's previous values exactly by
    ...
    void FluxLimiterNH4SH(){ FluxLimiter<cNeptuneModel>(m).nh4sh(); }

    // -----------------------------------------------------------------------
    void DiffMassFluxNept()
    {
        using namespace std;
        cout << endl << "      ATNEPT: DiffMassFluxNept" << endl;
        ...
        for(int k = 1; k < km-1; k++){
            for(int j = 1; j < jm-1; j++){
                for(int i = 1; i < im-1; i++){
                    ...
                    m.jT_nh3.x[i][j][k]   = m.DT_nh3   / m.t.x[i][j][k] * dtdr;  // Soret-effect (thermodiffusion)
                    m.jT_nh4sh.x[i][j][k] = m.DT_nh4sh / m.t.x[i][j][k] * dtdr;  // Soret-effect (thermodiffusion)

                    // Pole-symmetric sum of gradient components: for a pole-symmetric
                    // scalar field c, dcdr and dcdphi are pole-symmetric while dcdthe
                    // is pole-antisymmetric. Using std::abs() on the theta term makes
                    // the resulting scalar pole-symmetric for symmetric inputs.
                    const double dh2s   = dh2sdr   + std::abs(dh2sdthe)/rm   + dh2sdphi/rmsinthe;
                    const double dnh3   = dnh3dr   + std::abs(dnh3dthe)/rm   + dnh3dphi/rmsinthe;    // Fick's diffusion (concentration gradient)
                    const double dnh4sh = dnh4shdr + std::abs(dnh4shdthe)/rm + dnh4shdphi/rmsinthe;  // Fick's diffusion (concentration gradient)

                    m.j_h2s.x[i][j][k]   = m.c_mix/m.r_mix * (m.m_h2s * m.D_h2s * dh2s
                                          - m.jT_h2s.x[i][j][k]);
                    m.j_nh3.x[i][j][k]   = m.c_mix/m.r_mix * (m.m_nh3 * m.D_nh3 * dnh3  // combined diffusion flux, higher to lower concentration
                                          - m.jT_nh3.x[i][j][k]);
                    m.j_nh4sh.x[i][j][k] = m.c_mix/m.r_mix * (m.m_nh4sh * m.D_nh4sh * dnh4sh  // combined diffusion flux, higher to lower concentration
                                          - m.jT_nh4sh.x[i][j][k]);
                }
            }
        }
        ...
        for(int k = 1; k < km-1; k++){
            for(int j = 1; j < jm-1; j++){
                for(int i = 1; i < im-1; i++){
                    ...
                    // See Pass 1 comment: theta-derivative is pole-antisymmetric for
                    // pole-symmetric inputs, so wrap it in std::abs() so that the
                    // divergence-like scalars and the temperature-gradient factor
                    // below remain pole-symmetric.
                    m.difflux_h2s.x[i][j][k]   = dj_h2sdr   + std::abs(dj_h2sdthe)/rm   + dj_h2sdphi/rmsinthe;  // mass transport by diffusion
                    ...
                    m.thermalmassflux.x[i][j][k] =                      // enthalpy transport by diffusion
                          (m.j_nh3.x[i][j][k]   * m.cp_nh3              // heat flux by mass transport by diffusion
        ...
        auto end = std::chrono::high_resolution_clock::now();
        auto elapsed = std::chrono::duration_cast<std::chrono::nanoseconds>(end - begin);
        printf(" time measured: %.3f seconds for DiffMassFluxNept\n", elapsed.count() * 1e-9);

        cout << "      ATNEPT: DiffMassFluxNept ended" << endl;
        return;
    }

    // -----------------------------------------------------------------------
    void ThermalPropertiesNept()
    {
        using namespace std;
        cout << endl << "      ATNEPT: ThermalPropertiesNept" << endl;
        ...
        // MASS-FRACTION WEIGHTING, which is ATJUP's form (ChemistryJup.h:441) and was not ATNEPT's.
        //
        ...
        //
        // ON NEPTUNE THIS MOVES THE INITIAL PROFILE, because gam is not a config constant here:
        // cNeptuneModel.cpp:228 computes gam = g*1e3/cp_mix immediately before init_temperature,
        // and init_temperature builds T_bottom = T_top + gam*L_atm. So the deep equator was
        // 254 K too hot purely from the weighting:
        //     cp_mix        6380.3192059532 -> 8683.24... J/(kg K)   +36.1 %
        //     gam           1.7397 -> 1.2783 K/km
        //     deep equator  1012.4 -> 758.6 K   (T_top 55.5 K + gam*550 km)
        //
        // THIS ANSWERS THE QUESTION LEFT OPEN BY 44dde95 ("whether 1111 K at 550 km depth is right
        // for Neptune at all"). Much of it was this. The t_max_K() = 2000 ceiling set there stays
        // clear of the corrected profile too, so it remains a guard rather than a limiter.
        // ATSAT and ATURAN carry the same defect and are fixed alongside.
        ...
        // DiffMassFluxNept refreshes this every call with the current p_stat and t.
        ...
        auto end = std::chrono::high_resolution_clock::now();
        auto elapsed = std::chrono::duration_cast<std::chrono::nanoseconds>(end - begin);
        printf(" time measured: %.3f seconds for ThermalPropertiesNept\n", elapsed.count() * 1e-9);

        cout << "      ATNEPT: ThermalPropertiesNept ended" << endl;
        return;
    }
