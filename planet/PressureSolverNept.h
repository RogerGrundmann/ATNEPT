 ...
 * Standalone pressure-solver class for the Neptune model.
...
public:
    explicit PressureSolverNept(cNeptuneModel& model)
        : m(model)
    {}
    ...
    {
        using namespace std;
        cout << endl << "      ATNEPT: computePressure" << endl;
        ...
        // red-black colouring. This solver is the DEFAULT (ATNEPT_PRESS_SOLVER=0) and never got
        // it, so the fix landed in the path almost nothing runs. ATSAT and ATJUP escaped it by
        // DELETING their per-planet solvers and binding straight to the shared one; ATURAN had
        // the identical defect and fixed it the identical way.
        //
        // MEASURED, nm=4, single run of this build. With the pragma: two 16-thread runs differ in
        // 1 of 7 output files, and 1 vs 16 threads differ in 6 of 7. Serial: 1 thread and 16
        // threads agree BIT-IDENTICALLY, two 16-thread runs agree bit-identically, and all of
        // them agree bit-identically with the 1-thread answer this loop gave BEFORE the change,
        // with not one differing log line — one thread ran collapse(2) in lexicographic order
        // already, so serialising takes nothing back. EVERY single-threaded measurement in
        // README.md still stands unchanged.
        //
        // Serial rather than red-black because it costs almost nothing: computePressure is 0.003 s
        // at 16 threads and 0.03 s serial. Red-black would change the answer and strand every
        // measurement taken so far to recover 30 milliseconds. ATNEPT_PRESS_SOLVER=1 still
        // selects the shared red-black solver for anyone who wants this loop parallel.
        //
        ...
        cout << "      ATNEPT: computePressure ended" << endl;
    }
