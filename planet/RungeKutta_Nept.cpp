...
void cNeptuneModel::RungeKuttaNept(){
    cout << endl << "      ATNEPT: RungeKuttaNept" << endl;
    double
    ...
    for(int i = 1; i < im-1; i++){
        for(int j = 1; j < jm-1; j++){
            ...
            // Build geometry struct once per (i,j) row — reused for all k
            CellGeometry geo;
            geo.rm      = rad.z[i];
            ...
            for(int k = 1; k < km-1; k++){
                ...
                cNeptuneModel::RHSNept(i, j, k, geo);
                ...
                h2s_cloud.x[i][j][k] = h2s_cloudn.x[i][j][k] + kh2s_cloud1 * 0.5 * dt;
                h2s_ice.x[i][j][k]   = h2s_icen.x[i][j][k]   + kh2s_ice1   * 0.5 * dt;
                ...
                h2s_cloud.x[i][j][k] = h2s_cloudn.x[i][j][k] + kh2s_cloud2 * 0.5 * dt;
                h2s_ice.x[i][j][k]   = h2s_icen.x[i][j][k]   + kh2s_ice2   * 0.5 * dt;
                ...
                h2s_cloud.x[i][j][k] = h2s_cloudn.x[i][j][k] + kh2s_cloud3 * dt;
                h2s_ice.x[i][j][k]   = h2s_icen.x[i][j][k]   + kh2s_ice3   * dt;
                ...
                t.x[i][j][k] = AtomUtils::clamp(
                    tn.x[i][j][k] + dt * (kt1 + 2.0*kt2 + 2.0*kt3 + kt4) / 6.0, t_min, t_max);
                u.x[i][j][k] = un.x[i][j][k] + dt * (ku1 + 2.0*ku2 + 2.0*ku3 + ku4) / 6.0;
                v.x[i][j][k] = vn.x[i][j][k] + dt * (kv1 + 2.0*kv2 + 2.0*kv3 + kv4) / 6.0;
                w.x[i][j][k] = wn.x[i][j][k] + dt * (kw1 + 2.0*kw2 + 2.0*kw3 + kw4) / 6.0;

                h2o.x[i][j][k] = h2on.x[i][j][k]
                    + dt * (kc1 + 2.0*kc2 + 2.0*kc3 + kc4) / 6.0;
                h2o_cloud.x[i][j][k] = h2o_cloudn.x[i][j][k]
                    + dt * (kcloud1 + 2.0*kcloud2 + 2.0*kcloud3 + kcloud4) / 6.0;
                h2o_ice.x[i][j][k] = h2o_icen.x[i][j][k]
                    + dt * (kice1 + 2.0*kice2 + 2.0*kice3 + kice4) / 6.0;

                h2s.x[i][j][k] = h2sn.x[i][j][k]
                    + dt * (kh2s1 + 2.0*kh2s2 + 2.0*kh2s3 + kh2s4) / 6.0;
                h2s_cloud.x[i][j][k] = h2s_cloudn.x[i][j][k]
                    + dt * (kh2s_cloud1 + 2.0*kh2s_cloud2 + 2.0*kh2s_cloud3 + kh2s_cloud4) / 6.0;
                h2s_ice.x[i][j][k] = h2s_icen.x[i][j][k]
                    + dt * (kh2s_ice1 + 2.0*kh2s_ice2 + 2.0*kh2s_ice3 + kh2s_ice4) / 6.0;

                nh3.x[i][j][k] = nh3n.x[i][j][k]
                    + dt * (knh31 + 2.0*knh32 + 2.0*knh33 + knh34) / 6.0;
                nh3_cloud.x[i][j][k] = nh3_cloudn.x[i][j][k]
                    + dt * (knh3_cloud1 + 2.0*knh3_cloud2 + 2.0*knh3_cloud3 + knh3_cloud4) / 6.0;
                nh3_ice.x[i][j][k] = nh3_icen.x[i][j][k]
                    + dt * (knh3_ice1 + 2.0*knh3_ice2 + 2.0*knh3_ice3 + knh3_ice4) / 6.0;

                ch4.x[i][j][k] = ch4n.x[i][j][k]
                    + dt * (kch41 + 2.0*kch42 + 2.0*kch43 + kch44) / 6.0;
                ch4_cloud.x[i][j][k] = ch4_cloudn.x[i][j][k]
                    + dt * (kch4_cloud1 + 2.0*kch4_cloud2 + 2.0*kch4_cloud3 + kch4_cloud4) / 6.0;
                ch4_ice.x[i][j][k] = ch4_icen.x[i][j][k]
                    + dt * (kch4_ice1 + 2.0*kch4_ice2 + 2.0*kch4_ice3 + kch4_ice4) / 6.0;

                nh4sh.x[i][j][k] = nh4shn.x[i][j][k]
                    + dt * (knh4sh1 + 2.0*knh4sh2 + 2.0*knh4sh3 + knh4sh4) / 6.0;
            }
        }
    }
    ...
    cout << "      ATNEPT: RungeKuttaNept ended" << endl;
    return;
}
/*
*
