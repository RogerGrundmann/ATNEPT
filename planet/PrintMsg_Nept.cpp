...
void cNeptuneModel::printMinMax(){
    ...
    // makes: zero unless ATNEPT_RADIATION is set, printed regardless, so that switching the knob
    ...
    // number known about ATNEPT's radiation so far came from the one stdout line in the shared
    ...
    // model that had it.
    //
    // IT IS THE TABLE THAT WOULD HAVE CAUGHT THIS MODEL'S OWN 607x PRESSURE DEFECT ON THE FIRST
    // RUN, which is why it is here now. The photosphere was reported at 17.2287 bar and read as an
    // opacity failure for two commits; what was actually wrong was that init_PressureStatic put the
    // TOP of the domain at 15 bar, so tau = 1 was reached in the first layer below the ceiling. A
    // single column of p[bar] from top to bottom shows that immediately, and no amount of
    // column-MEAN diagnostics does. The p[bar] column earns its place here even with the radiation
    // knob off.
    {
