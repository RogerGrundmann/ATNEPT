...
//
// All are ZERO unless the module that fills them is on (ATNEPT_PRECIP, ATNEPT_TURB). They are
...
//    dump_array("Q_Sensible", Q_Sensible, 1.0, Neptune_panorama_vts_File);
    ...
    // unless ATNEPT_RADIATION is set.
    dump_array("Q_rad_mW_m3", Q_rad, 1.0e3, Neptune_panorama_vts_File);
    dump_array("Radiation", radiation, 1.0, Neptune_panorama_vts_File);
    dump_array("Emissivity", epsilon, 1.0, Neptune_panorama_vts_File);
    DUMP_TURB_PRECIP_VTS(Neptune_panorama_vts_File);
...
void cNeptuneModel::paraview_vtk_radial(int n, int i_radial){
   ...
   dump_radial("massflux_nh4sh", massflux_nh4sh, 1e9, i_radial, Neptune_vtk_radial_File);
    ...
    // both identically zero unless ATNEPT_RADIATION is set.
    dump_radial("Q_rad_mW_m3", Q_rad, 1.0e3, i_radial, Neptune_vtk_radial_File);
    dump_radial("Radiation", radiation, 1.0, i_radial, Neptune_vtk_radial_File);
    dump_radial("Emissivity", epsilon, 1.0, i_radial, Neptune_vtk_radial_File);
    DUMP_TURB_PRECIP_3D(dump_radial, i_radial, Neptune_vtk_radial_File);
...
void cNeptuneModel::paraview_vtk_zonal(int n, int k_zonal){
    ...
    // Radiation pair, matching ATJUP and ATSAT. Q_rad in mW/m3, radiation in W/m2;
    // both identically zero unless ATNEPT_RADIATION is set.
    dump_zonal("Q_rad_mW_m3", Q_rad, 1.0e3, k_zonal, Neptune_vtk_zonal_File);
    dump_zonal("Radiation", radiation, 1.0, k_zonal, Neptune_vtk_zonal_File);
    dump_zonal("Emissivity", epsilon, 1.0, k_zonal, Neptune_vtk_zonal_File);
    DUMP_TURB_PRECIP_3D(dump_zonal, k_zonal, Neptune_vtk_zonal_File);
...
void cNeptuneModel::paraview_vtk_longal(int n, int j_longal){
    ...
    // Radiation pair, matching ATJUP and ATSAT. Q_rad in mW/m3, radiation in W/m2;
    // both identically zero unless ATNEPT_RADIATION is set.
    dump_longal("Q_rad_mW_m3", Q_rad, 1.0e3, j_longal, Neptune_vtk_longal_File);
    dump_longal("Radiation", radiation, 1.0, j_longal, Neptune_vtk_longal_File);
    dump_longal("Emissivity", epsilon, 1.0, j_longal, Neptune_vtk_longal_File);
    DUMP_TURB_PRECIP_3D(dump_longal, j_longal, Neptune_vtk_longal_File);
