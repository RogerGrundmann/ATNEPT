...
void SaturationAdjustmentNept::run_legacy(const std::string& gas,
        ...
        for(int j = 0; j < jm; j++){
            for(int i = 0; i < im; i++){
                ...
                if(cell_found){
                    ...
                    // differed once the pressure-solver race was fixed in e353df7.
                    //
 ...
 * Default is the inherited one; ATNEPT_SATADJ=1 selects the shared algorithm. See the header for
 * what is known about the difference (measured on ATSAT, not yet on Neptune) and for why the ice
 * quadruple below is the liquid one.
 */
void SaturationAdjustmentNept::run(const std::string& gas,
    double coeff_A,   double coeff_B,
    double coeff_A_i, double coeff_B_i,
    double t_0,       double t_00,
    double ep,        double lv,  double ls,
    ...
    Array& c,         Array& cloud,   Array& ice)
{
    ...
    // when real Neptune ice coefficients exist; the shared algorithm already treats the liquid
    // and ice pairs separately, so nothing else has to move.
    SaturationAdjustment<cNeptuneModel>(m).run(gas, t_0, t_00, ep, lv, ls,
